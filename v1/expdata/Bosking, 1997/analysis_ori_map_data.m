...
DATA_OPMAP = imread('Data/RAW_OPMAP.jpg');
% RGB - OPMAP Conversion
res = 500;
OP_MAP = analysis_RGB_ori_mapping(res,DATA_OPMAP,false);
% Gaussian Noise Reduction
SIG = 20; OP_MAP_F = angle(imgaussfilt(real(exp(2i*OP_MAP)),SIG)+1i*imgaussfilt(imag(exp(2i*OP_MAP)),SIG))/2;
NEW_OP_MAP = OP_MAP_F;
OP_MAP_F(OP_MAP_F<0) = OP_MAP_F(OP_MAP_F<0)+pi;
% figure;
% subplot(221); image(DATA_OPMAP); axis image; title("RGB data image");
% subplot(223); imagesc(OP_MAP); axis image; colormap(hsv); colorbar; title("Orientation image");
% subplot(224); imagesc(OP_MAP_F); axis image; colormap(hsv); colorbar; title("Filtered OP image");
